...
export default function LoadingProvider({ children }: { children: ReactNode }) {
    ...
    const [queue, setQueue] = useState<queueItem[]>([
        {
            id: 2,
            name: 'shelves',
            message: 'Wait a bit, let us load shelves and its items for you...'
        },
        {
            id: 1,
            name: 'design',
            message: 'Now, we\'re implementing nice styles for you...'
        },
    ])
    ...
    return (
        <LoadingContext.Provider value={{ queue, setQueue }}>
            <LoadingComponent ref={loaderRef} message={queue.length > 0 ? queue[0].message : undefined} />
